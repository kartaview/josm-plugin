...
/**
 * Defines the {@code KartaViewDetailsDialog} action panel. The user can perform the following actions: filter, jump to
 * photo location and open photo web page.
 *
 * @author Beata
 * @version $Revision$
 */
class ButtonPanel extends JPanel implements NearbyPhotoObservable, LocationObservable, SequenceObservable,
    ...
    private void addSwitchPhotoFormatButton() {
        ...
        if (PreferenceManager.getInstance().loadPhotoSettings().isDisplayFrontFacingFlag()) {
            ...
            btnSwitchImageFormat = ButtonBuilder.build(action, IconConfig.getInstance().getFrontFacingImageFormatIcon(),
                    tooltip, false);
        }
    ...
    /**
     * Enables or disables the "KartaView Sequence" related action buttons based on the given arguments. The play button
     * is disabled if the last photo of the track was selected (isNext is false).
     *
    ...
    private final class SelectPhotoAction extends JosmAction {
        ...
        private SelectPhotoAction(final String shortcutText, final boolean isNext) {
            super(shortcutText, null, shortcutText, ShortcutFactory.getInstance().getShortcut(shortcutText), true);
            this.isNext = isNext;
        }
    ...
    private final class ClosestPhotoAction extends JosmAction {
        ...
        private ClosestPhotoAction() {
            super(GuiConfig.getInstance().getBtnClosestShortcutText(), null,
                    ...
                    ShortcutFactory.getInstance().getShortcut(GuiConfig.getInstance().getBtnClosestShortcutText()),
                    true);
        }
    ...
    private final class TrackAutoplayAction extends JosmAction {
        ...
        private TrackAutoplayAction() {
            super(GuiConfig.getInstance().getBtnPlayShortcutText(), null,
                    ...
                    ShortcutFactory.getInstance().getShortcut(GuiConfig.getInstance().getBtnPlayShortcutText()), true);
        }
    ...
    private final class JumpToLocationAction extends JosmAction {
        ...
        private JumpToLocationAction() {
            super(GuiConfig.getInstance().getBtnLocationShortcutText(), null,
                    ...
                    ShortcutFactory.getInstance().getShortcut(GuiConfig.getInstance().getBtnLocationShortcutText()),
                    true);
        }
    ...
    private final class OpenWebPageAction extends JosmAction {
        ...
        private OpenWebPageAction() {
            super(GuiConfig.getInstance().getBtnWebPageShortcutTlt(), null,
                    ...
                    ShortcutFactory.getInstance().getShortcut(GuiConfig.getInstance().getBtnWebPageShortcutTlt()),
                    true);
        }
        ...
        public void actionPerformed(final ActionEvent event) {
            if (photo != null) {
                final StringBuilder link = new StringBuilder(KartaViewServiceConfig.getInstance().getPhotoDetailsUrl());
