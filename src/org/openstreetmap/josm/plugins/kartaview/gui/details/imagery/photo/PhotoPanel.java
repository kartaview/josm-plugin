...
import org.openstreetmap.josm.plugins.kartaview.entity.PixelPoint;
import org.openstreetmap.josm.plugins.kartaview.observer.DetectionSelectionObservable;
...
class PhotoPanel extends JPanel implements MouseWheelListener, DetectionSelectionObservable {
    ...
    private void drawOnWrappedImage(final Graphics2D graphics, final Detection detection, final Photo displayedPhoto) {
        if (detection != null && displayedPhoto != null && detection.getShapeOnPhoto() != null
                && detection.getShapeOnPhoto().getEquirectangularPolygon() != null
        ...
        public void mouseClicked(final MouseEvent e) {
            if (detections != null && image != null) {
                ...
                if (DataSet.getInstance().isFrontFacingDisplayed()) {
                    ...
                    selectedDetection = detections.stream().filter(detection -> detection.getShapeOnPhoto() != null
                            && detection.getShapeOnPhoto().getEquirectangularPolygon() != null && detection
                            .getShapeOnPhoto().isPointInEquirectangularPolygon(translatedPoint)).findFirst()
                            .orElse(null);
                }
