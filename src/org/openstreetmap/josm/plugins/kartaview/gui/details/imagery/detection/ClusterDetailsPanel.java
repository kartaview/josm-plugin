...
class ClusterDetailsPanel extends BaseDetailsPanel<Cluster> {
    ...
    protected void createComponents(final Cluster cluster) {
        ...
        final int widthLbl = getMaxWidth(getFontMetrics(getFont().deriveFont(java.awt.Font.BOLD)),
                ...
                GuiConfig.getInstance().getClusterMatchingConfidenceLbl(),
                GuiConfig.getInstance().getClusterConfidenceCategoryLbl(),
        ...
        if (cluster.getConfidenceLevel() != null) {
            if (cluster.getConfidenceLevel().getOcrConfidence() != null) {
                addInformation(GuiConfig.getInstance().getOcrConfidenceLbl(), EntityFormatter
                                .formatDouble(cluster.getConfidenceLevel().getOcrConfidence(), false,
                                        DecimalPattern.MEDIUM),
                        widthLbl);
            }
            if (cluster.getConfidenceLevel().getMatchingConfidence() != null) {
                addInformation(GuiConfig.getInstance().getClusterMatchingConfidenceLbl(), EntityFormatter
                                .formatDouble(cluster.getConfidenceLevel().getMatchingConfidence(), false,
                                        DecimalPattern.MEDIUM),
                        widthLbl);
            }
        }
        if (Objects.nonNull(cluster.getConfidenceLevel())) {
            addInformation(GuiConfig.getInstance().getClusterConfidenceCategoryLbl(),
                    cluster.getConfidenceLevel().getConfidenceCategory(), widthLbl);
        }
        ...
        if (detection != null && table != null && table.getCluster() != null) {
            ...
            for (int rowIndex = 0; rowIndex < table.getRowCount(); ++rowIndex) {
                if (table.getValueAt(rowIndex, ID_COLUMN) != null) {
                    final long currentRowId = Long.parseLong(table.getValueAt(rowIndex, ID_COLUMN).toString());
