...
import org.openstreetmap.josm.plugins.kartaview.entity.DetectionConfidenceLevel;
...
class DetectionDetailsPanel extends BaseDetailsPanel<Detection> {
    ...
    private void addConfidenceLevel(final DetectionConfidenceLevel confidenceLevel, final int widthLbl) {
        if (confidenceLevel != null && confidenceLevel.isNotNull()) {
            if (confidenceLevel.getConfidenceCategory() != null) {
                addInformation(GuiConfig.getInstance().getClusterConfidenceCategoryLbl(),
                        confidenceLevel.getConfidenceCategory(), widthLbl);
            }
